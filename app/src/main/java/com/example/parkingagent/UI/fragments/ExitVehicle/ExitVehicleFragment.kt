...
@AndroidEntryPoint
class ExitVehicleFragment : BaseFragment<FragmentMoreBinding>() {
    ...
    private val viewModel: ExitVehicleViewModel by viewModels()
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.mutualSharedflow.collectLatest {
                    when (it) {
                        is ExitVehicleViewModel.ParkingVehicleEvents.VehicleParkingSuccessful -> {
                            showToast(it.vehicleParkingResponse.msg.toString())
                            clearFields()
                        }

                        is ExitVehicleViewModel.ParkingVehicleEvents.VehicleParkingFailed -> {
